...
@pytest.mark.parametrize("amount", [-50.0, 1000.0])
...
def test_deposit_negative_amount():
    account = BankAccount("Tom&Jerry", 100)
    with pytest.raises(ValueError):
        account.deposit(-50)

def test_deposit_non_number():
    account_2 = BankAccount("Captain_Balu", 100)
...
def test_transfer_insufficient_funds():
    account_1 = BankAccount("Tom&Jerry", 100)
    account_2 = BankAccount("Captain_Balu", 100)
    with pytest.raises(ValueError):
        account_1.transfer(150, account_2)
