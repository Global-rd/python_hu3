...
town_name = input("Dear Sarah. Please type in the town name: ").title().strip()                                        # nagybetűs lesz
#rent_price = int(input("Give me the max rent price $: ")) if town_name not in ["Chicago", "Washington"] else 0  # csak ha kell
if town_name not in ["Chicago", "Washington"]:
    rent_price = int(input("Give me the max rent price $: "))
...
elif (town_name in ["New York", "San Francisco"] and rent_price < 4000) \
    or (town_name not in ["New York", "San Francisco"] and rent_price <= 3000): 
